...
datExprT <- fread("GTEx_frontal_cortex_TPM_SampleNetworks/All_09-32-34/GTEx_frontal_cortex_TPM_All_186_Qnorm.csv", data.table=FALSE)
...
subgroup1 <- grep("DTHHRDY", colnames(sampleinfo1)) # grep("SMCENTER", colnames(sampleinfo1))
...
btrait1 <- c(
  ...
  grep("SMTSISCH", colnames(sampleinfo1)),
  grep("SMRIN", colnames(sampleinfo1)),
  grep("SMTS", colnames(sampleinfo1)),
...
SampleNetwork(
  ...
  cexlabels1=0.7,
  normalize1=FALSE, # DON'T NORMALIZE AGAIN
  replacenegs1=FALSE,
  exportfigures1=TRUE,
  verbose=TRUE
)
